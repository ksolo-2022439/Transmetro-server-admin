...
export const validateCreateRoad = [
  ...
  body("stations")
    ...
    .isArray()
    .withMessage(
      "Las estaciones deben enviarse como un arreglo (puede estar vacío o contener códigos/IDs)",
    ),
  body("coordinates")
    .isArray({ min: 2 })
    .withMessage(
      "Las coordenadas deben ser un arreglo de al menos 2 puntos (LineString)",
    ),
  body("coordinates.*")
    .isArray({ min: 2, max: 2 })
    .withMessage(
      "Cada punto de la ruta debe ser un arreglo [longitud, latitud]",
    ),
  body("coordinates.*.*")
    .isFloat()
